...
const LoginWithPhone = () => {
  ...
  useEffect(() => {
    ...
    const userphonenumber = sessionStorage.getItem("User_key");

    if (userphonenumber) {
      setShowOtpField(false);
      setregisterNumber(userphonenumber);
    }
    const checkType = async () => {
      const { data } = await axios.get(`/api/v2/type?exam_type=${exam_type}`);
      if (data.success) {
      ...
      <div class="otp_wraper">
        <div class="otp_form">
          ...
          <div class="otp_content text-center">
            ...
            {showOtpField ? <div className='number-container'>
              <input style={{ flex: '0 0 75%', margin: '0px' }}
                onChange={(e) => setNumber(e.target.value)}
                value={getNumber}
                type="text"
                placeholder="Enter Phone Number here"
              ...
              <button style={{ margin: '0px' }} className="btn" onClick={getUserInfo}>
                SEND OTP
              </button>
              ...
              : <div className='number-container'>
                <input style={{ flex: '0 0 73%', margin: '0px' }}
                  onChange={(e) => setOtp(e.target.value)}
                  value={otp}
                  type="text"
                  placeholder="Enter Otp here"
                ...
                <button style={{ margin: '0px' }} className="btn" type="submit" onClick={verifyUser}>
                  VERIFY OTP
                </button>
              </div>
