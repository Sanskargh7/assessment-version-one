...
class User {
  ...
  static login = async (req, res) => {
    ...
    const mobile = phone.replaceAll(" ", "").slice(-10);
    if (!otp) {
  ...
  static calculateUserScore = async (req, res) => {
    ...
    try {
      ...
      const getUserData = await AnswerModel.aggregate([
        {
          $match: {
            $and: [{ userId: userId }, { examType: examType }]
          },
        },
        {
          $sort: {
      ...
      //check existing user info in user result table👍✔
      const isExistUser = await UserResultModel.findOne({ $and: [{ userId: userId }, { examType: examType }] });
      ...
      if (!isExistUser) {
        ...
        // if (storedUser) {
        //   const isSendEmail = await UserModel.findOne({ _id: userId });
        //   const sendEmail = await SettingsModel.findOne({ settingsName: 'emailTemplate' })
        //   const email = isSendEmail.email;
        //   if (sendEmail.send_result_email) {
        //     sendEmailTemplate(storeUserScore, sendEmail, email)
  ...
  //Get  All exams of userId
  static getAllExamsByUser = async (req, res) => {
    try {
      ...
      if (getAllExams) {
        ...
        return res.status(200).json({ success: false, data: 'data not found!' })
      }
  ...
  static isValidExam = async (req, res) => {
    try {
      ...
      if (validatePassCode) {
        ...
        return res.status(200).json({ success: false, msg: "Invalid PassCode!" })
      }
    }
    catch (error) {
      return res.status(500).json({ success: false, error: error.message })
